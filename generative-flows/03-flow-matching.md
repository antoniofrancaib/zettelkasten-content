...
## 3.1 The Continuity Equation: Probability as Fluid
...
> [!remark] 3.1
> The continuity equation (3.1) is the cornerstone of the entire theory. In fluid mechanics, it governs the conservation of mass. In kinetic theory, it governs the evolution of particle distributions. In the context of generative modeling, it says precisely what is required of a velocity field: at every point and time, the velocity must be consistent with how the probability density is changing. Every normalizing flow, every diffusion model, and every flow matching method is implicitly solving the continuity equation — they differ only in how they parametrize the solution.


## 3.2 Flow Matching: Design Your Own Path
...
**Flow matching inverts this.** Choose a desired probability path $\{p_t\}_{t \in [0,1]}$ first — explicitly, deliberately, with $p_0 = \mathcal{N}(0, I)$ and $p_1 = p_{\text{data}}$. Then derive the target vector field $u_t$ from the continuity equation (3.1), and train $v_\theta$ to match it by regression. No ODE simulation is needed during training: evaluating the regression target requires only sampling from the path, not integrating the ODE.

> [!definition] 3.1 — Flow Matching Objective
...
The problem: $u_t(x)$ is the **marginal** velocity field, defined as the minimum-norm solution of (3.1). For an arbitrary $p_t$, this requires computing a conditional expectation over all data points that could have contributed to the density at $x$ — a high-dimensional integral that is no more tractable than the likelihood computation it was meant to replace.

> [!equation] 3.2
> $$u_t(x) = \mathbb{E}_{p_1(x_1 | x_t = x)}\!\left[u_t(x \mid x_1)\right],$$


where $u_t(x|x_1)$ is the conditional velocity field at $x$, given that the trajectory will end at $x_1$. To evaluate (3.2), you would need to invert the flow at every point — again, an ODE solve. The marginal flow matching objective is formally correct but computationally inert.

## 3.3 The Conditional Flow Matching Theorem
...
> [!theorem] 3.1 — Conditional Flow Matching Equivalence
...
>
> Consequently, $\mathcal{L}_{\mathrm{FM}}$ and $\mathcal{L}_{\mathrm{CFM}}$ have the same minimizer, and minimizing the tractable conditional objective (3.3) produces a vector field that exactly transports $p_0$ to $p_1$.
...
## 3.4 Gaussian Conditional Paths and Their Velocity Fields
...
With $\sigma_{\min} \to 0$, we have $x_t = t\, x_1 + (1-t)\, \epsilon$ and, substituting into (3.4):
...
an objective with the same tower-property justification as Theorem 3.1 — the regression target is an unbiased sample of the conditional expectation $u_t(x_t)$.

> [!definition] 3.2 — Stochastic Interpolant
> A **stochastic interpolant** is a parametric family of probability paths $\{p_t\}$ defined by (3.6). The family subsumes:
...
## 3.6 Rectified Flow: Straight Lines and Reflow
...
where $\pi_0 = p_0 \times p_1$ is the independent coupling — each noise sample paired randomly with each data sample. This is the same objective as (3.5) with $\epsilon = x_0$ and $x_1 = $ data. At this level, rectified flow and linear flow matching are identical.
...
## 3.7 Optimal Transport Conditioning
...
> [!proposition] 3.2 — OT Conditioning and Path Straightness
...
## 3.8 Diffusion Models as a Special Case

The unification theorem — that diffusion models are a special case of flow matching — is now straightforward to state. The probability flow ODE from Chapter 2 (equation 2.8):

$$\frac{dx}{dt} = f(x, t) - \tfrac{1}{2}g(t)^2\, \nabla_x \log p_t(x),$$

is a flow with a specific vector field $u_t(x) = f(x,t) - \frac{1}{2}g(t)^2\nabla_x\log p_t(x)$. This vector field satisfies the continuity equation (3.1) for the marginals $\{p_t\}$ of the forward diffusion SDE. It is therefore a valid marginal velocity field in the sense of flow matching — one that arises from a particular choice of probability path.

What probability path? For the VP-SDE, the marginal at time $t$ is $p_t(x) = \int p_{\text{data}}(x_1)\mathcal{N}(x;\sqrt{\bar\alpha_t}x_1, (1-\bar\alpha_t)I)dx_1$ — a Gaussian convolution of the data distribution that shrinks the signal and inflates the noise. This is exactly the stochastic interpolant (3.6) with $\alpha_t = \sqrt{\bar\alpha_t}$, $\beta_t = 0$, $\sigma_t = \sqrt{1-\bar\alpha_t}$.
...
## 3.9 The Practical Consequences
...
## 3.10 What the Convergence Reveals
...
**The remaining chapters build on this foundation.** Chapter 4 develops the transport-theoretic underpinnings: optimal transport theory behind path straightening, Schrödinger bridges, and entropic transport. Chapter 5 explores the SDE–ODE duality and drift-based generation. Chapter 6 subsumes everything under the generator matching framework. Chapter 7 extends to discrete and Riemannian spaces. Chapters 8 and 9 address the remaining challenges of efficient sampling, Monte Carlo correction, and guided generation.
...
**Lipman, Chen, Ben-Hamu, Nickel, and Le (ICLR 2023)** ("Flow Matching for Generative Modeling") introduced the conditional flow matching theorem (Theorem 3.1) and Gaussian conditional paths, establishing the general framework. **Albergo and Vanden-Eijnden (ICLR 2023)** ("Building Normalizing Flows with Stochastic Interpolants") introduced the stochastic interpolant framework (Definition 3.2), emphasizing the role of the coupling between $x_0$ and $x_1$ and the connection to stochastic analysis. **Liu, Gong, and Liu (ICLR 2023)** ("Flow Straight and Fast: Learning to Generate and Transfer Data with Rectified Flow") introduced the reflow procedure and established the connection between coupling straightness and inference efficiency.
